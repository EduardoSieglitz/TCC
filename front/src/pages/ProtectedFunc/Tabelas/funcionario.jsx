...
const TabelaFuncionario = () => {
  ...
  const [editUserId, setEditUserId] = useState(null); // Estado para gerenciar edição
  ...
  const handleDelete = async (idFuncionario) => {
    try {
      await axios.delete(`http://localhost:3001/funcionarios/${idFuncionario}`);
  ...
  const handleSave = async (data) => {
    try {
      const response = await axios.put(`http://localhost:3001/funcionarios/${editUserId}`, data);
